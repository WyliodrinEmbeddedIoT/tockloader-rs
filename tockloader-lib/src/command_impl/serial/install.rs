...
impl CommandInstall for SerialConnection {
    async fn install_app(
        ...
        //     let _ = ping_bootloader_and_wait_for_response(stream).await?; // doesnt return anything anymore
        // }
        ...
        match board {
            ...
            Err(e) => {
                return Err(TockloaderError::Internal(
                    InternalError::MisconfiguredBoardSettings(e),
                ));
            }
        }
        ...
        match arch {
            Ok(arch) => {
                ...
                match binary {
                    Ok(mut binary) => {
                        ...
                        let _ =
                            issue_command(stream, Command::ErasePage, pkt, true, 0, Response::OK)
                                .await?;
                    }
                    Err(e) => {
                        return Err(e);
            ...
            Err(e) => Err(TockloaderError::Internal(
                InternalError::MisconfiguredBoardSettings(e),
            )),
        }
    }
