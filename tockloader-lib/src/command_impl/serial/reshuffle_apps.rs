...
#[async_trait]
impl UtilityReshuffleApps for SerialConnection {
    async fn reshuffle_apps(
        ...
        let mut installed_apps: Vec<AppAttributes> =
            AppAttributes::read_apps_data_serial(stream, settings.start_address)
                .await
        ...
        for app in installed_apps.iter() {
            match app.installed {
                true => {
                    ...
                    // log::info!("reading at addr {:#x}, size {} and name {}", app.address, app.size, app.tbf_header.get_package_name().unwrap_or(""));
                    let mut pkt = (app.address as u32).to_le_bytes().to_vec();
                    let length = (app.size as u16).to_le_bytes().to_vec();
                    for i in length {
                        pkt.push(i);
                    }
                    // log::info!("sending pkt {:?}", pkt);
                    let (_, mut appdata) = issue_command(
                        stream,
                        Command::ReadRange,
                        pkt,
                        true,
                    ...
                    while appdata.len() < app.size as usize {
                        appdata.push(0u8);
                    }
                    // log::info!("binary size {}", appdata.len());
                    // log::info!("found appdata {:?} of size {}", appdata, appdata.len());
                ...
                false => {
                    let aux = tab
                        ...
                        .unwrap();
                    // log::info!("binary len {}", aux.len());
                    app_binaries.push(aux);
                }
            }
        }
        // panic!();

        for (index, app) in installed_apps.iter().enumerate() {
            log::debug!(
                "{}, {} - start {:#x}, size {}",
                index,
        ...
        // log::info!("saved is");
        for item in saved_configuration.iter() {
            // log::info!("{}, {} - start {:#x}, size: {}", index, item.tbf_header.get_package_name().unwrap_or(""), item.address, item.size);
            ...
            if item.is_padding {
                ...
                binary.append(&mut buf);
            } else {
                binary.append(&mut app_binaries[item.index as usize]);
            }
        }
        ...
        while (page_number + 1) * PAGE_SIZE <= binary.len() as u32 {
            let mut pkt = (settings.start_address as u32 + page_number * PAGE_SIZE)
                .to_le_bytes()
                .to_vec();
            // log::debug!("writing page {}", page_number);
            // log::info!("writing at {:#x}", settings.start_address as u32 + page_number * PAGE_SIZE);
            // log::info!("taking slice from {} to {}", (page_number * PAGE_SIZE) as usize, ((page_number + 1) * PAGE_SIZE));
            pkt.append(
                &mut binary
                    [(page_number * PAGE_SIZE) as usize..((page_number + 1) * PAGE_SIZE) as usize]
                    .to_vec(),
            );
            // log::info!("created pkt {:?}", &pkt[0..20]);
            let (_, _) =
        ...
        // erase the last page
        // log::info!("calling erase");
        let pkt = (settings.start_address as u32 + binary.len() as u32)
            .to_le_bytes()
            .to_vec();

        let _ = issue_command(stream, Command::ErasePage, pkt, true, 0, Response::OK).await?;
        log::info!("(Serial): Finished reshuffle apps");
        Ok(())
    }
}
