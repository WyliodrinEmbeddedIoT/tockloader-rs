...
impl CommandInstall for ProbeRSConnection {
    async fn install_app(
        ...
        // reshuffle is enough for installing c_apps (so far)
        ...
        //     }
        // }
        // pkt.append(&mut [0xFFu8; 512].to_vec());
        // log::info!("full buffer is {:?}", pkt);
        // log::info!("writing at {:#x}", new_address);
        // let mut loader = session.target().flash_loader();

        // loader.add_data(new_address.into(), &pkt)?;
