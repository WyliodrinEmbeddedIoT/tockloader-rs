...
impl VirtualTerminal for SerialInterface {
    ...
    async fn run_terminal(&mut self) -> Result<(), TockloaderError> {
        ...
        let (writer, reader) = LinesCodec::new().framed(self.stream.take().unwrap()).split();
                        ...
                        match buffer.as_str() {
                            "\x08" => { // Backspace
                                if line_buffer.len() > 0 {
                                    line_buffer.pop();
                                    print!("\x08 \x08"); // Erase last char visually
                                }
                            }
                            "" => { // Enter pressed
                                println!(); // Move to next line
                                print!("tockloader$ ");
                                // print!("enter pressed, this is the buffer before clear -{}-", line_buffer);
                                if let Some(writer) = writer_arc.upgrade() {
                                    writer.lock().await.send(line_buffer.clone()).await.unwrap();
                                }
                                line_buffer.clear();
                            }
                            _ => {
                                print!("{}", buffer);
                                line_buffer.push_str(&buffer);
                            }
                        }
